...
int iothub_mqtt_client_run(void)
{
    ...
    else
    {
        ...
        IOTHUB_CLIENT_RETRY_POLICY retryPolicy = IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF_WITH_JITTER;
        ...
        const char* publishData_2 = "publish 1st hello world message to node topic 2";

        result = publish_mqtt_message(clientHandle, TOPIC_NAME_B, DELIVER_AT_MOST_ONCE, (const uint8_t*)publishData_2,
                                      strlen(publishData_2), NULL , NULL);

        const char* publishData_1 = "publish 1st hello world message to node topic 1";

        result = publish_mqtt_message(clientHandle, TOPIC_NAME_A, DELIVER_AT_LEAST_ONCE, (const uint8_t*)publishData_1,
                                      strlen(publishData_1), pub_least_ack_process , clientHandle);
